...
class RosGpsEkf:
	...
	callback_lock = False
	husky_ekf_odom_pub = rospy.Publisher('/odom/ekf', Odometry, queue_size=1)
	...
	def ekf_reset(self):
		...
		self.Q = np.mat(np.diag([0.1, 0.1, 1000, 0.1]))
		self.Q_init = copy.deepcopy(self.Q)
		self.P = np.mat(np.diag([0.5, 0.5, 10, 0.5])) # z to 0 for 2D localization
		# self.P = np.mat(np.diag([0, 0, 0]))
		self.R = np.mat(np.diag([10, 10, 0.001, 10]))
		self.R_init = copy.deepcopy(self.R)

		# self.R = np.mat(np.diag([2, 2, 3]))

		self.F = np.mat(np.diag([1.0, 1.0, 1.0, 1.0]))
		self.H = np.mat(np.diag([1, 1, 1, 1])) # z to 0 for 2D localization
		self.X = np.zeros((4, 1))
	...
	def husky_model(self, x, y, theta, z, vel, alpha, dt):
		...
		theta = theta + alpha*self.alpha_gain*dt
	...
	def abs_odom_callback(self, msg):
	...
	def vehicle_state_callback(self, msg):
		...
		new_vel = msg.twist.twist.linear.x
		new_alpha = msg.twist.twist.angular.z
	...
	# Yaw and Yaw
		...
		self.husky_ekf_odom_pub.publish(msg)


if __name__ == '__main__':
	rospy.init_node('husky_ekf_node')

	husky_ekf = RosGpsEkf()

	husky_ekf.frame_id = rospy.get_param('~frame_id', 'odom')
	husky_ekf.child_frame_id = rospy.get_param('~child_frame_id', 'ekf')

	husky_ekf.shift = rospy.get_param('~shift', -0.0)
	# husky_ekf.publish_odom_tf = rospy.get_param('~publish_odom_tf', True)

	# rospy.Subscriber("/odom/sbg", Odometry, husky_ekf.abs_odom_callback, queue_size = 1)
	rospy.Subscriber("/odom/slam", Odometry, husky_ekf.abs_odom_callback, queue_size = 1)
	rospy.Subscriber("/odom/odometry", Odometry, husky_ekf.vehicle_state_callback, queue_size = 1)

	# rospy.Subscriber('/vehicle/motion_command', MotionCommandStamped3, husky_ekf.motion_command_callback, queue_size=1)
