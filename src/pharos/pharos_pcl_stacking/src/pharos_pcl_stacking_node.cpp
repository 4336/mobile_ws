...
void VlpPointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr &msg){
	...
	try{
		listener.lookupTransform("vehicle_frame", "velodyne", ros::Time(0), transform);
	}catch (tf::TransformException ex){
		ROS_ERROR("%s",ex.what());
		ros::Duration(0.01).sleep();
	}
	pcl_ros::transformPointCloud("vehicle_frame", transform, *msg, output);
...
void updatePCL()
{
	...
	try{
		listener2.lookupTransform("vehicle_frame", "odom", ros::Time(0), odom2rover);
	}catch (tf::TransformException ex){
		ROS_ERROR("%s",ex.what());
		ros::Duration(0.01).sleep();
	}
	...
	*CloudStack += *CloudTemp;
	...
	stacked_msg.header.frame_id = "vehicle_frame";
...
int main(int argc, char **argv)
{
	...
	sub_rover_odom = nh.subscribe("/odom/vehicle", 10, RoverOdomCallback); //topic que function
