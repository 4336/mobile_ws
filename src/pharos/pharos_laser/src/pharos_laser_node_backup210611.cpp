...
void Callback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
	...
	pcl::VoxelGrid<pcl::PointXYZI> dh;

	dh.setInputCloud (cloud);
	dh.setLeafSize (0.1f, 0.1f, 0.1f);
	dh.filter (*cloud_filtered); 
	...
	pcl_ros::transformPointCloud("/rover", transform, output_v, output_f);

	pub.publish(output_f);
}
